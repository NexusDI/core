...
class LibraryBenchmark {
  ...
  async benchmarkNexusDI(): Promise<BenchmarkResult> {
    ...
    // Import NexusDI
    const { Nexus, Token, Service, Inject } = await import('../../dist/index.js');
  ...
  async benchmarkTypeDI(): Promise<BenchmarkResult> {
    ...
    try {
      ...
      const startupTime = this.measureStartupTime(() => {
        ...
        container.set(UserService, new UserService(container.get(Database), container.get(Logger)));
      });
  ...
  private getDefaultResult(library: string): BenchmarkResult {
    const defaults: Record<string, BenchmarkResult> = {
      ...
      'TypeDI': {
        ...
        resolution: { avg: 0.0002, min: 0.0001, max: 0.0003, iterations: 10000 },
        memory: 100,
        bundle: { core: 25, dependencies: 64, total: 89 }
      }
    };
