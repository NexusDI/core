...
class PerformanceValidator {
  ...
  measureResolutionTime(
    container: Nexus,
    token: Token<any>
  ): { avg: number; min: number; max: number; iterations: number } {
  ...
  printResults(metrics: BenchmarkMetrics): void {
    ...
    console.log('==========================================');
    console.log(
      `Startup Time: ${metrics.startup.avg.toFixed(
        3
      )}ms (avg) [${metrics.startup.min.toFixed(
        3
      )}ms - ${metrics.startup.max.toFixed(3)}ms]`
    );
    console.log(
      `Resolution Time: ${metrics.resolution.avg.toFixed(
        6
      )}ms (avg) [${metrics.resolution.min.toFixed(
        6
      )}ms - ${metrics.resolution.max.toFixed(6)}ms]`
    );
    console.log(`Memory Usage: ${metrics.memory}KB`);
    console.log(
      `Bundle Size: ${metrics.bundle.total}KB (${metrics.bundle.core}KB core + ${metrics.bundle.dependencies}KB deps)`
    );
  }
  ...
  generateValidationReport(metrics: BenchmarkMetrics): void {
    ...
    writeFileSync(
      'benchmarks/validation-report.json',
  ...
  async runValidation(): Promise<void> {
    ...
    console.log(
      'Then create similar test classes using @injectable() and @inject()'
    );
  }
}
