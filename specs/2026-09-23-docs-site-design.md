...
- The benchmarks-and-launch spec: the benchmark harness and the results JSON it writes. Every
  performance figure on the site comes from that file (section 4.3).
...
`/comparison/` presents four measured benefits as its concrete case: bundle size, startup
time, resolve time and build time. Its H2 "Why NexusDI needs no compiler flags" is the
site's answer to that question, and it cites the same four measurements, since dropping
`experimentalDecorators`, `emitDecoratorMetadata` and `reflect-metadata` is what the build
time and bundle figures measure. Every figure on either comes from the results JSON of the
benchmark harness (benchmarks-and-launch spec). `docs:bench-results` copies that file into
`apps/docs/generated/bench-results.json` before the build, and the page renders each
figure through `<BenchFigure metric="…" library="…" />`, which reads the file and shows the
figure with the harness version, the machine and the run date beside it. A metric the file
does not hold fails the build. No figure appears without a harness source, and
`doc-bench-figures` enforces that (section 14.3).
...
| `docs:bench-results`    | `tools/bench-results.mjs`    | `generated/bench-results.json`, copied from the benchmark harness's results                                                                  |
...
| `doc-bench-figures`       | new                          | On `/comparison/`, and on any page that mounts `<BenchFigure>`, no prose, table cell or fence states a number followed by a size or time unit (`B`, `kB`, `KB`, `MB`, `µs`, `ms`, `s`) outside a `<BenchFigure>`. Every `<BenchFigure>` names a metric and library that `bench-results.json` holds.                                                                                                                                                                                                                                                                           |
