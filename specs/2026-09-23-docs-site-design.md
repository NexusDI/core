...
3. Migration is a first-class band with seven pages (section 4.3). 0.3.1 has users, and the
...
1. What `set()` did in 0.3: it registered one provider into a mutable container, at any
   time, before or after the first `get()`.
2. Why 0.4 has none: `Nexus.create` validates the whole graph before it builds anything,
   so every provider is declared in a module, and the container is sealed once `create`
   resolves (core spec §2.3).
3. What `load(Module)` does: it adds a whole module after startup. It validates the module
   against the live graph, builds the module's singletons, awaits their async factories,
   runs their `onInit`, and then publishes the new graph. It does nothing for a module the
   root already imports, and concurrent calls run one at a time, in call order (core spec
   §3.5).
4. Why `load()` is not `set()` under a new name: it takes a module, where `set()` took one
   token; it is async; and a per-token `set()` after `create` would add a provider the
   compiler never validated, which is the failure the sealed container exists to prevent.
5. What each 0.3 `set()` call becomes. A `set()` at setup time becomes an entry in a
   module's `providers`, and the codemod's `container-bootstrap` transform does that
   rewrite. A `set()` after startup becomes a module plus `ship.load()`, and the codemod
...
`/react-router-ssr/` and the code-splitting guide `/load/` follow the integrations spec
(`specs/2026-09-23-integrations-design.md` §3.9). Both load a code-split section with the
